...
begin
  ...
  select coalesce(jsonb_object_agg(v.component, v.version), '{}'::jsonb)
  into latest_versions
  from public.system_component_versions v
  where v.component in (
    'app',
  ...
  app_version := coalesce(latest_versions ->> 'app', '0.0.0');
  compute_gateway_extraction_version :=
    coalesce(latest_versions ->> 'compute_gateway_extraction', '0.0.0');
  embedding_pipeline_version := coalesce(latest_versions ->> 'embedding_pipeline', '0.0.0');
  extraction_pipeline_version := coalesce(latest_versions ->> 'extraction_pipeline', '0.0.0');
  indexing_pipeline_version := coalesce(latest_versions ->> 'indexing_pipeline', '0.0.0');
  inngest_indexing_workflow_version :=
    coalesce(latest_versions ->> 'inngest_indexing_workflow', '0.0.0');
  tree_indexer_version := coalesce(latest_versions ->> 'tree_indexer_python', '0.0.0');
  tree_prompt_version := coalesce(latest_versions ->> 'tree_prompt', '0.0.0');
